...
import {DrawLayerMode} from '../../../models/drawTypes';

const RIGHT_PANE_WIDTH = 50;
...
const styles = StyleSheet.create({
  ...
  rightPaneBaseStyle: {
    ...
    right: 0,
    // @ts-ignore
    top: 100 + STATUS_BAR_HEIGHT,
    bottom: 100 + FIX_IPHONE_X_BOTTOM_SPACE,
    // top: (130 * 812) / deviceHeight,
    // bottom: (130 * 812) / deviceHeight,
...
>(
  (
    {showDrawTool, drawingMode, image, linearGradient, onSelectionChange},
    ref,
  ) => {
    const enabledDraw = useSharedValue<DrawLayerMode>('show');
    const mode = useSharedValue<DrawItemType>('pen');
    const [drawRegion, setDrawRegion] = useState<Size | null>(null);

    const [originalImageSize, setOriginalImageSize] = useState<Size | null>(
      null,
    );
    ...
    useEffect(() => {
      enabledDraw.value = showDrawTool;
    }, [enabledDraw, showDrawTool]);
    ...
    return (
      <View style={styles.container}>
        <View
          ...
          <PanGestureHandler
            enabled={enabledDraw.value === 'draw'}
            activeOffsetX={2}
            activeOffsetY={2}
